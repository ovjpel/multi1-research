...
def generate_response(prompt, api_handler):# Get the absolute path to the system_prompt.txt file
    ...
    max_context = 4096 # max context length (originally was hardcoded at 300)

    # Main loop for generating reasoning steps
    while True:
        # Measure time taken for each API call
        start_time = time.time()
        step_data = api_handler.make_api_call(messages, max_context)
        end_time = time.time()
        thinking_time = end_time - start_time
        total_thinking_time += thinking_time

        # Store each step's information
        print(f"\n[utils: generate_response()] Raw step_data straight out of API call: {step_data}")
    ...
    if os.getenv("SAVE_OUTPUT", "No") == "Yes":
        ...
        try:
            ...
            with open(output_file, "a", newline='', encoding='utf-8') as f:
                ...
                if file_is_empty:
                    writer.writerow(["Prompt", "Step Title", "Step Content", "Thinking Time (s)"])

                for step in steps:
                    writer.writerow([prompt, step[0], step[1], f"{step[2]:.2f}"])
            print(f"[utils: generate_response()] Output successfully saved to {output_file}")
        except Exception as e:
            print(f"[utils: generate_response()] Failed to save output to {output_file}: {e}")
